  ...
  _createClass(MetingTag, [{
      ...
      optionalArgs.forEach(function (option, index) {
        switch (true) {
          ...
          case option.startsWith('api:'):
            settings.api = (0, _util.extractOptionValue)(option);
            break;
          default:
            (0, _util.throwError)('Unrecognized tag argument(' + (index + 1) + '): ' + option);
        }
      });
      return settings;
    }
    ...
    value: function generate() {
      ...
      Object.entries(this.settings).forEach(function (_ref) {
        ...
        if (key === 'api') {
          var val = value.includes(':server') ? value : value.replace(/\/?$/, '/') + '?server=:server&type=:type&id=:id&r=:r';
          settingLiteral += ' data-api="' + (0, _util.escapeHtml)(val) + '"';
        } else {
          settingLiteral += ' data-' + key + '="' + (0, _util.escapeHtml)(value) + '"';
        }
      });
